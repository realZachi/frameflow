...
- **Stack**: Vercel AI SDK v7 (`streamText` with `tools` and `stopWhen: isStepCount(64)`), Anthropic provider `@ai-sdk/anthropic`, model `claude-sonnet-5`. Calls go directly from the browser to the Anthropic API; this requires the `anthropic-dangerous-direct-browser-access: true` header, set in `createAnthropic` in `runner.ts`. The key is read from `import.meta.env.VITE_ANTHROPIC_API_KEY`, configured in `.env.local` (never committed).
