...
Reasoning-effort choices belong to the model catalog. Offer only values supported by the selected model and pass non-default choices through the AI SDK's top-level `reasoning` option; do not duplicate provider-specific effort mapping in the UI. The generate modal exposes one model picker grouped by provider; reasoning effort appears as secondary chips only when the selected model supports it.
